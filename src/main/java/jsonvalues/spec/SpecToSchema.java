...
public final class SpecToSchema {
    ...
    static {
        ...
        LogicalTypes.register(BIGINTEGER_LOGICAL_TYPE, schema -> {
            if (schema.getType() != Schema.Type.STRING)
        ...
        LogicalTypes.register(ISO_FORMAT_LOGICAL_TYPE, schema -> {
            if (schema.getType() != Schema.Type.STRING)
    ...
    public static Schema convert(final JsObjSpec spec) throws SpecNotSupportedInAvroException {
        if (spec.metaData == null) throw MetadataNotFoundException.errorParsingJsObSpecToSchema();
    ...
    private static JsValue toJsSchema(JsSpec spec,
        ...
        if (spec instanceof JsArrayOfStrSuchThat) return arrayOfStringSchema(spec, defaultValue);
        if (spec instanceof JsArrayOfStr) return arrayOfStringSchema(spec, defaultValue);
        if (spec instanceof JsArrayOfTestedStr) return arrayOfStringSchema(spec, defaultValue);
    ...
    private static boolean containsInstant(JsArray schema) {
        for (JsValue value : schema) {
            var isInstant = value.isObj(obj -> obj.getStr("type").equals("string") && ISO_FORMAT_LOGICAL_TYPE.equals(obj.getStr("logicalType")));
    ...
    private static boolean containsDecimal(JsArray schema) {
        for (JsValue value : schema) {
            var isInstant = value.isObj(obj -> obj.getStr("type").equals("string") && BIG_DECIMAL_LOGICAL_TYPE.equals(obj.getStr("logicalType")));
            if (isInstant) return true;
        }
        return false;
    }
    ...
    private static boolean containsBigInteger(JsArray schema) {
        for (JsValue value : schema) {
            var isInstant = value.isObj(obj -> obj.getStr("type").equals("string") && BIGINTEGER_LOGICAL_TYPE.equals(obj.getStr("logicalType")));
            if (isInstant) return true;
        }
        return false;
    }
    ...
    private synchronized static JsValue objSpecSchema(JsObjSpec objSpec,
                ...
                fieldSchema =
                        fieldSchema.set(DEFAULT_FIELD,
                                        (keyDefault instanceof JsBinary b) ?
                                                JsStr.of(new String(b.value, StandardCharsets.UTF_8)) : keyDefault
                                       );
            }
            if (aliases != null)
        ...
        cacheSchema(metadata.getFullName(), schema);
    ...
    private static JsValue namedSchema(String name, boolean nullable, JsValue keyDefault) {
        return nullable ?
                (keyDefault.isNull() ?
                        JsArray.of("null", name) :
                        JsStr.of(name)
                ) : JsStr.of(name);
    }
    ...
    private synchronized static JsValue fixedSchema(JsFixedBinary js, JsValue defaultValue) {
    ...
    private synchronized static JsValue enumSchema(JsEnum jsEnum, JsValue defaultValue) {
        ...
        if (metaData.namespace() != null)
            schema = schema.set(NAMESPACE_FIELD, JsStr.of(metaData.namespace()));
        if (metaData.aliases() != null)
